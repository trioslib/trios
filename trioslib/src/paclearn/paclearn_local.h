...
xpl_t *            /*+ Purpose: builds an example's set from a pair of input-output
        images that represents a binary-to-grayscale transformation. The examples
        collecting process can be gathered by a mask image.                         +*/

  collec_BG(

    unsigned short *s1,     /*+ In: input image data              +*/
    unsigned short *s2,     /*+ Out: output image data            +*/
...
xpl_t *   /*+ Purpose: builds an example's set from a pair of input-output
    images that represents a gray-scale-to-gray-scale transformation. The examples
        collecting process can be gathered by a mask image.
...
xpl_t *      /*+ Purpose: builds an example's set from a pair of input-output
   ...
   level restriction. The examples collecting process can be gathered by a mask 
   image. These three images are considered gray-scale ones.                     +*/
  collec_WKF(
    unsigned char *p1, /*+ In: input image data                                  +*/
    unsigned char *p2, /*+ Out: output image data                                +*/
    unsigned char *p3, /*+ In: mask image data                                   +*/
...
/*
     -------------------------------------------
     FUNCTION: collec_WKC - Classifier
     -------------------------------------------
*/
...
/*
     -------------------------------------------
     FUNCTION: nbands_collec
     -------------------------------------------
*/

xpl_t * /*+ Purpose: builds an example's set from a pair of input-output
        images that represents a gray-scale-to-gray-scale transformation 
        with gray-level restriction. The examples collecting process can 
        be gathered by a mask image. These three images are considered 
        gray-scale ones.                                                        +*/

  nbands_collec(
    unsigned char  *p1,  /*+ In: input image data                               +*/
    unsigned char  *p2,  /*+ In: output image data                              +*/
    unsigned char  *p3,  /*+ In: mask image data                                +*/
    ...
    int   *ko,           /*+ In: the output graylevel range                     +*/
    int   *vplace,       /*+ In: vertical placement of the window               +*/
    int   nbands_i,      /*+ In: number of bands to be processed                +*/
    int   nbands_o       /*+ In: number of bands to be observed                 +*/
) ;
/*+ Returns: an example's set on success, NULL on failure +*/ 
...
xpl_t * /*+ Purpose: builds an example's set from a pair of input-output
  ...
  nbands_collec_WK2(
    ...
    unsigned char  *p3,  /*+ In: mask image data                          +*/
    window_t *win,       /*+ In: window                                   +*/
    int   *offset,       /*+ In: offset (contains window information)	  +*/
    int   npixels,       /*+ In: number of pixels in the image            +*/
    int   nbands_i       /*+ In: number of bands to be processed          +*/
    ) ;
...
int                  /*+ Purpose: Makes the decision for BG                 +*/
  decide_tree_bg_moda(
    xpl_BG *p,       /*+ In: Begining  of the examples                      +*/
    mtm_t *mtm,      /*+ In/Out:  Pointer to minterms                       +*/
    unsigned int   sum,       /*+ In: Total number of examples                       +*/
...
int                  /*+ Purpose: Makes the decision BG                     +*/
  decide_tree_bg_general(
    xpl_BG *p,       /*+ In: Begining  of the examples                      +*/
    mtm_t *mtm,      /*+ In/Out:  Pointer to minterms                       +*/
    unsigned int   sum,       /*+ In: Total number of examples                       +*/
    int   wzip,      /*+ In: wpat size in bytes                             +*/
...
int                 /*+ Purpose: ISI with groupment of examples            +*/
  isi_gen(
...
itv_BX *        /*+ Purpose: Given an interval I, and a point X, computes
  ...
  itv_split_bywpat(
    itv_BX *I,        /*+ In: interval to be splitted                      +*/
    unsigned int *wpat,
...
int         /*+ Purpose: Perform ISI by points (binary case)           +*/
  isi_basic(
...
int             /*+ Purpose: remove intervals that do not contain any
                             example of a given set of classified examples +*/
  itv_filter(                                                                 
...
int             /*+ Purpose: create auxiliar structure to hold intervals   +*/
  itv2mp(                                                                    
    itv_t *itv,       /*+ In/Out: set of intervals                         +*/
...
itv_BX *        /*+ Purpose: select essential intervals                    +*/
  essential_sel(                                                           
    mtx_t **mx_head,   /*+ In/Out: list of intervals                       +*/
    mtm_t *mtm,        /*+ In: classified examples set                     +*/
...
int             /*+ Purpose: given pointers for intervals and examples,
                    ...
                    used for removing the dominated rows (intervals)       +*/
  set_matrix(                                                                    
...
int             /*+ Purpose: delete, from a list of indices, those that
                   ...
                   difference is the returning value)                      +*/
  del_covered(                                                                 
    mtm_t *mtm,        /*+ In: set of classified examples                  +*/
...
/*
     -------------------------------------------
     FUNCTION: DECIDE_OTHER
     -------------------------------------------
*/

mtm_t       /*+ Purpose: Builds a set of classified examples from a set of
                   observed examples, taking into consideration in the
                   decision process, an inertia factor                     +*/
 *decide_other(
   xpl_t *xpl,      /*+ In: pointer to a set of examples                   +*/
   int   conflt,    /*+ In: label to be assigned to undecidable examples   +*/
...
/*
     -------------------------------------------
     FUNCTION: decide_ll_bb_other
     -------------------------------------------
*/
 
int                  /*+ Purpose: Makes the decision based on an inertia
		       factor for examples of BB type. Assign conflt code to
                       conflicting examples                                 +*/
  decide_ll_bb_other(
    xpl_t *xpl,      /*+ In: set of examples                                +*/
    mtm_t *mtm,      /*+ In/Out: classified examples                        +*/
    int   lambda,    /*+ In: factor of inertia                              +*/
    int   conflt     /*+ In: Value assigned to conflicting examples         +*/
) ;
/*+ Return: 1 on Success, 0 on Failure                                      +*/ 
...
int             /*+ Purpose: update T1 (computes T1[i+1] from T1[i])       +*/
  update_T1(                                                                  
...
/*
     ...
     -------------------------------------------
*/
int             /*+ Purpose: update T0 (computes T0[1] to T0[cur])         +*/
  update_T0(                                                                   
    graph_t *graph,    /*+ In/Out: Inversion set represented by a graph    +*/
    int  cur,
    idx_ll **garbage
...
int           /*+ Purpose: given a list of root nodes, removes from it
                    all nodes that are feasible (i.e., has negative cost)  +*/
  clea_root_list(                                                            
...
void            /*+ Purpose: to free memory used by a graph structure      +*/
  graph_free(                                                                 
    graph_t *graph     /*+ In/Out: pointer to a graph structure            +*/
  ) ;
/*+ Return: nothing                                                        +*/
...
int             /*+ Purpose: find the inversion set (and its complement)   +*/
  pinvset(                                                                    
    mtm_t  *mtm,       /*+ In: classified examples set                     +*/
...
int             /*+ Purpose: during the graph building process, given an
                    ...
                    edges linking it to the parent nodes found             +*/
  set_parent(                                                                 
...
graph_t *      /*+ Purpose: build the traversing graph of an inversion set +*/
  build_graph(                                                                
    mtm_t **mtm,    /*+ In/Out: set of classified examples                 +*/
...
void            /*+ Purpose: print graph (for debugging)                   +*/
  print_graph(                                                                
...
int             /*+ Purpose: compute the switching costs for the nodes of
                    a graph, based on a beta probability density           +*/
  gcomp_cost(                                                                 
...
int             /*+ Purpose: write some info about the switching set
  ...
  swt_output_log(                                                             
    ...
    unsigned long int time_usr1,
    unsigned long int time_sys1,
    unsigned long int time_usr2,
    unsigned long int time_sys2,
    unsigned long int time_usr3,
    unsigned long int time_sys3,
    unsigned long int time_usr4,
    unsigned long int time_sys4
) ;
/*+ Return: 1 on success, 0 on failure                                     +*/
...
int             /*+ Purpose: separate the set of classified examples in
                disjoint subsets, according to the decomposition vaiables  +*/
  sep_mtm(                                                                    
    mtm_t *mtm,        /*+ In: classified examples                         +*/
    itv_t *itv,        /*+ In: interval                                    +*/
...
xpl_t *            /*+ Purpose: Threshold gray-level image and collects
		     binary w-patterns                            +*/
 colthresh_BB(
    unsigned short *s1,     /*+ In: input image data              +*/
    unsigned char  *p2,     /*+ Out: output image data            +*/

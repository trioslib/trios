	...
	while (!stop) {
		...
		case 'A':
			if (apt != NULL && NULL == (*apt = apert_read_data(fd))) {
				fclose(fd);
				return (xpl_t *) trios_error(MSG,
							     "xpl_read: apert_read_data() failed.");
			}
			break;
		case 'd':
			stop = 1;
...
#ifdef _DEBUG_2_
			trios_debug("bwpat=%x fq0=%d fq1=%d", bwpat[0], fq0,
				    fq1);
#endif
		...
		for (i = 0; i < n_nodes; i++) {
			...
			do {
				...
				if (freq) {
					...
					if ((freqnode =
					     freq_node_create(label,
							      freq)) == NULL) {
						free(wpat);
						fclose(fd);
						return (xpl_t *)
						    trios_error(MSG,
								"read_xpl: freq_node_create() failed.");
					}
#ifdef _DEBUG_2_
					trios_debug("freqnode criado");
					trios_debug("label=%d , freq=%d\n",
			...
			if (xpl_GG_insert
			    (xpl, (xpl_GG **) (&xpl->root), wpat,
			     freqlist) == -1) {
				fclose(fd);
				return (xpl_t *) trios_error(MSG,
							     " read_xpl : xpl_GG_insert() failed.");
			}
		}
		free(wpat);
		break;

...
int write_multi_architecture_data(FILE * f, char *filename,
				  multi_architecture_t * arch)
{
	...
	fprintf(f, "\n");
	for (k = 0; k < arch->nlevels; k++) {
		fprintf(f, "%d %d\n", arch->levels[k].ninputs,
			arch->levels[k].noperators);
		for (i = 0; i < arch->levels[k].noperators; i++) {
			for (j = 0; j < arch->levels[k].ninputs; j++) {
				sprintf(cmd,
					"mkdir -p %s-files/level%d/operator%d-files",
					filename, k, i);
				system(cmd);

				if (k == 0) {
					sprintf(window_name,
						"%s-files/level%d/operator%d-files/window",
						filename, k, i);
				} else {
					sprintf(window_name,
						"%s-files/level%d/operator%d-files/window%d",
						filename, k, i, j);
				}

				win_write(window_name,
					  arch->levels[k].windows[i][j]);
			}
		}
	}
	return 1;
...
int write_multi_architecture_data2(FILE * f, char *filename,
				   multi_level_operator_t * op)
{
	int i, j, k;
	char window_name[1024], cmd[1024];
...
multi_architecture_t *read_multi_architecture_data(FILE * f, char *filename)
{
	...
	if (fscanf(f, "%d\n", &nlevels) != 1) {
		return (multi_architecture_t *) trios_error(MSG,
							    "Invalid multi_architecture file: %s nlevels",
							    filename);
	}
	trios_malloc(ops, sizeof(int) * nlevels, multi_architecture_t *,
		     "Failed to alloc memory for ops in m_arch_read.");
	for (k = 0; k < nlevels; k++) {
		if (fscanf(f, "%d ", ops + k) != 1) {
			return (multi_architecture_t *) trios_error(MSG,
								    "Invalid multi_architecture file: %s level",
								    filename);
		}
	}
	...
	for (k = 0; k < nlevels; k++) {
		if (fscanf
		    (f, "%d %d\n", &(march->levels[k].ninputs),
		     &(march->levels[k].noperators)) != 2) {
			return (multi_architecture_t *) trios_error(MSG,
								    "Invalid multi_architecture file: %s level %d",
								    filename,
								    k);
		}
		for (i = 0; i < march->levels[k].noperators; i++) {
			for (j = 0; j < march->levels[k].ninputs; j++) {
				...
				march->levels[k].windows[i][j] =
				    win_read(window_name);
			}
		}
	}
	return march;
	...
	for (k = 0; k < mop->nlevels; k++) {
		for (i = 0; i < mop->levels[k].noperators; i++) {
			for (j = 0; j < mop->levels[k].ninputs; j++) {
				sprintf(temp_name,
					"%s-files/level%d/operator%d-files/operator",
					filename, k, i);
				if (mop->type == BB) {
					joint =
					    multi_level_operator_joint_window
					    (mop, k, i);
					itv_write(temp_name,
						  (itv_t *) mop->levels[k].
						  trained_operator[i], joint);
					win_free(joint);
					if (k == 0) {

...
char *get_dir_name(char *path, char *read_dir_name)
{
	...
	if (i < 0) {
		...
		trios_malloc(result, sizeof(char) * (3 + dir_name_len), char *,
			     "Bad alloc");
		result[0] = '.';
		result[1] = '/';
		result[2] = '\0';
	} else {
		trios_malloc(result, sizeof(char) * (i + 2 + dir_name_len),
			     char *, "Bad alloc");
...
imgset_t *imgset_read(char *fname)
{
	...
	if (!(fd = fopen(fname, "r"))) {
		return (imgset_t *) trios_error(1, "File (%s) open failed.",
						fname);
	}
	...
	for (i = 1; i <= grpsize; i++) {
		...
		if (!imgset_set_dname(imgset, i, dir_name)) {
			return (imgset_t *) trios_error(MSG,
							"imgset_read: imgset_set_dname() failed.");
		}
		free(dir_name);
	}
	...
	for (k = 1; k <= ngroups; k++) {
		for (i = 1; i <= grpsize; i++) {
			...
			if (!imgset_set_fname(imgset, i, k, name)) {
				return
				    (imgset_t *) trios_error(MSG,
							     "imgset_read: imgset_set_fname() failed.");
			}
		}
	}
...
int imgset_create_write(char *fname, char *fname_obs, char *fname_ide,
			char *fname_mask)
{

  ...
  for(i=0; i<nmtm; i++) {
    ...
    while(p_list != NULL) {
      ...
      while((j<wzip) && (!stop_flag)) {
        if( ((p_list->A[j]&q[i].wpat[j])!=p_list->A[j]) ||
            ((p_list->B[j]&q[i].wpat[j])!=q[i].wpat[j]) ) {
	  stop_flag = 1 ;
        }
        j++ ;
      }
    ...
    while(p_kill) {
      ...
      p_split = itv_split_bywpat(p_kill, q[i].wpat, itv->wsize, wzip) ;
    ...
    tmp = p_new ;
    if(p_new) {
      while(p_new->next) p_new=p_new->next ;
      p_new->next = p_list ;
      p_list = tmp ;
    }
    itv->head = (int *)p_list ;
  ...
  * ---------------------------------------------------- */
  ...
  for(i=0; i<nmtm; i++) {
    ...
    while(p_kill) {
      ...
      while(ll_no) {
	ll_aux = ll_no->next ;
	if(itv_cover_wpat(p_list, q[ll_no->idx].wpat, wzip)) {
	  ll_no->next = ll_yes ;
	  ll_yes = ll_no ;
	}
	else {
	...
	ll_no = ll_aux ;
      ...
      if(!ll_no) {
        if(tmp0) {
          tmp = tmp0 ;
          while(tmp0->next) tmp0=tmp0->next ;
...
/*  date: Tue Dec 9 1997                                                    */
  ...
  * ---------------------------------------------------- */
  ...
  systemtime[ii3] = (float)(end_tms.tms_stime - start_tms.tms_stime)/100; 
  cusertime[ii3]  = (float)(end_tms.tms_cutime -start_tms.tms_cutime)/100;
  csystemtime[ii3]= (float)(end_tms.tms_cstime - start_tms.tms_cstime)/100;
...
int              /*+ Purpose: split all intervals of a list (of intervals)
                              by each interval in a second list             +*/
  gisi_0(
...
/* date: Thu Nov 21 1996                                                    */
  ...
  while(itv_list) {
    ...
    while(p != NULL) {
      ...
      while((j<wzip) && (!stop_flag)) {
	if(((p->A[j] | I->A[j])&(p->B[j] & I->B[j]))!=(p->A[j] | I->A[j])) {
	  stop_flag = 1 ;
	}
	j++ ;
      }
...
int     /*+ Purpose: split all intervals of a list (of intervals)
                     ...
                     (of the list to be removed) are processed             +*/
  gisi_3(

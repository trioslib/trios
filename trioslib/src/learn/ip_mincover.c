  ...
  while(p != NULL) {
    ...
    while((mp != NULL) && (counter <= 1)) {          
      ...
      while( (!stop_flag) && (j<size_of_zpat(mtm->wsize))) {
        if( ((mp->I->A[j] & q[p->idx].wpat[j]) != mp->I->A[j]) ||
            ((mp->I->B[j] & q[p->idx].wpat[j]) != q[p->idx].wpat[j]))  {
          stop_flag = 1 ;
        }
        j++ ;
      }
  ...
  while(cur) {
    ...
    while((!stop_flag) && (j<size_of_zpat(mtm->wsize))) {
      if( ((I->A[j] & q[cur->idx].wpat[j]) != I->A[j]) ||
          ((I->B[j] & q[cur->idx].wpat[j]) != q[cur->idx].wpat[j]) )  {
        stop_flag = 1 ;
      }
      j++ ;
    }
...
int             /*+ Purpose: add a list of intervals to a set of intervals
                       (does not check anything, does not copy).           +*/
  itv_add(                                                                    
    itv_t  *itv,       /*+ In/Out: set of intervals                        +*/
...
/*  date: Tue Dec 16 1997                                                   */
  ...
  while(p != NULL) {
    ...
    while(m != NULL) {
      ...
      while( (!stop_flag) && (j<size_of_zpat(mtm->wsize))) {
        if( ((m->I->A[j] & q[p->idx].wpat[j]) != m->I->A[j]) ||
            ((m->I->B[j] & q[p->idx].wpat[j]) != q[p->idx].wpat[j]))  {
          stop_flag = 1 ;
        }
        j++ ;
      }
...
/*  date: Tue Dec 16 1997                                                   */
  ...
  while(cur != NULL) {
    ...
    while((!flag) && (mp)) {
      ...
      while( (!stop_flag) && (j<size_of_zpat(mtm->wsize))) {
        if( ((mp->I->A[j] & q[x->idx].wpat[j]) != mp->I->A[j]) ||
            ((mp->I->B[j] & q[x->idx].wpat[j]) != q[x->idx].wpat[j]) )  {
          stop_flag = 1 ;
        }
        j++ ;
      }

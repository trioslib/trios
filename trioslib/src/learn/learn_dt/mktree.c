...
char dt_file[LINESIZE],
     train_data[LINESIZE];
char test_data[LINESIZE],
     misclassified_data[LINESIZE];
char log_file[LINESIZE];

int no_of_dimensions = 0,
    no_of_coeffs,
    no_of_categories = 0;
int no_of_restarts = 20,
    no_of_folds = 0;
int normalize = TRUE;
...
int no_of_stagnant_perturbations,
...
#ifdef _DEBUG_1_
pac_debug("dt_file=%s", dt_file) ;
#endif

  if (log_file_name) strcpy(log_file, log_file_name) ;  
  else strcpy(log_file,"dttrain.log");

  if ((logfile = fopen(log_file,"w")) == NULL) {
      (void) pac_error(1,"Mktree: Log file cannot be written to.\n") ;
      exit(0) ;
  }

#ifdef _DEBUG_1_
pac_debug("log_file=%s", log_file) ;
#endif
...
/* Rob: concordo, mas antes preciso dele para nao dar problemas com
  ...
  if(!(mtm = mtm_read(mtm_file, &win/*, &apt*/))) {
    return pac_error(MSG, "lpacdt: mtm_read() failed to read %s", mtm_file) ;
  }
...
pac_debug("Reading classified examples file: done.") ;
  ...
  for (i=0; i < mtm_get_nmtm(mtm); i++) {
    pac_debug("pattern %d = %d",i, mtm_GX_get_label(mtm, i)) ; 
  }
#endif
  ...
  pac_debug("no_of_train_points = %d", no_of_train_points) ;
  pac_debug("no_of_dimensions = %d", no_of_dimensions) ;
  pac_debug("no_of_categories = %d", no_of_categories) ;
#endif
  ...
  if (NULL == (map = (int *) malloc(sizeof(int)*(no_of_categories+1)))) {
    return pac_error(MSG,"ltrainDT: malloc failed for map") ;  
  }
  ...
  for (i=1; i <= no_of_categories; i++) {
    pac_debug("map[%d] = %d", i, map[i]) ;
  }
#endif



  if (NULL == (train_points = (struct point **) malloc(sizeof(int)*(no_of_train_points)))) {
    return pac_error(MSG,"ltrainDT: malloc failed for train_points") ;
  }

  for (i=0; i < no_of_train_points; i++) {
    if (NULL == (train_points[i] = (struct point *) malloc(sizeof(struct point)))) {
      return (int) pac_error(MSG,"ltrainDT: malloc failed for each train_point") ;
    }
  }
...
#ifdef _DEBUG_2_
pac_debug("pattern %d = %d",i,label_aux) ; 
#endif
...
#ifdef _DEBUG_2_
pac_debug("train_points[%d]->category = %d",i,train_points[i]->category) ;
#endif
  ...
  for (i=0; i < no_of_train_points ; i++) {
    if (train_points[i]->category != 0) {
      /*      for (j=1; j <= no_of_dimensions ; j++) { 
	pac_debug("%d ", pattern[j-1]) ;
	}*/
      pac_debug("label(%d) = %d", i, train_points[i]->category) ;
      pac_debug("(train_points[%d]->category = %d)", i, train_points[i]->category) ;
    }
  }
#endif
...
pac_debug("Before shuffle") ;
  ...
  if (no_of_missing_values) 
    fprintf(logfile,"%d missing values filled with respective attribute means.\n",
...
#ifdef _DEBUG_
pac_debug("Before allocate_structures") ;
#endif

  allocate_structures(no_of_train_points) ;

#ifdef _DEBUG_1_
  pac_debug("After allocate_structures") ;
#endif
...
pac_debug("deallocate_structures() done") ;
  ...
  pac_debug("user: %2d:%2d:%2d",(time_usr/3600),(time_usr%3600)/60, (time_usr%3600)%60);
  pac_debug("system: %2d:%2d:%2d",(time_sys/3600),(time_sys%3600)/60, (time_sys%3600)%60);
  pac_debug("Total: %2d:%2d:%2d",(time_sys+time_usr)/3600,((time_sys+time_usr)%3600)/60, ((time_sys+time_usr)%3600)%60 );
#endif
  
#ifdef _VERBOSE_
  for (i=1; i<= no_of_categories; i++) {
    if (result.class[2*i] != 0) {
      fprintf(logfile,"Category %d : accuracy = %.2f (%d/%d)\n",
     ...
     int size,prev_size ;
{
  ...
  if (prev_size == 0) {
    ...
    if (!array_name)
      pac_fatal("Allocate_Point_Array: Memory Allocation Failure 1.") ;
    ...
    for (i=1; i <= size; i++) {
      ...
      if (!array_name[i])
	pac_fatal("Allocate_Point_Array : Memory Allocation failure 2.") ;
    }
    ...
    if (!array_name)
      pac_fatal("Allocate_Point_Array: Memory Allocation Failure 3.") ;
    ...
    for (i=prev_size+1; i <= size; i++) {
      array_name[i] = (struct point *)malloc((unsigned) sizeof(struct point)) ; 
      if (!array_name[i])
	pac_fatal("Allocate_Point_Array : Memory Allocation failure 4.") ;
    }
...
/************************************************************************/
int allocate_structures(no_of_points)
     int no_of_points ;
{
...
#ifdef _DEBUG_
pac_debug("First four free done") ;
#endif
...
#ifdef _DEBUG_
pac_debug("First seven free done") ;
#endif
...
/************************************************************************/
int shuffle_points(array_name, count)
     POINT **array_name ;
     int count ;
{
  int i,
...
/* Remarks :                                                            */
/************************************************************************/
struct tree_node *build_tree(points, no_of_training_points, dt_file_local)
     struct point **points ;
     ...
     char *dt_file_local;
{
  ...
  */
  ...
  if (strlen(dt_file_local))  { 
    ...
    if (proot == root) printf("Unpruned decision tree written to %s.\n",dt_file_local) ;
  }
                  ...
                  int cur_no_of_points ;
{
  ...
  float oblique_split(),
        ...
        cart_split() ;
  */
  float initial_impurity,
        cur_impurity ;

  char lnode_str[MAX_DT_DEPTH],
       rnode_str[MAX_DT_DEPTH];


#ifdef _DEBUG_
  pac_debug("Entered build_subtree()") ;
#endif
  ...
  if (strlen(node_str)+1 >  MAX_DT_DEPTH)  {
    ...
    fprintf(stderr,"Depth cannot be more than MAX_DT_DEPTH, set in pacdt.h.\n") ;
    return(NULL) ;
  }
...
#ifdef _DEBUG_
  pac_debug("build_subtree: entering cart_mode") ;
#endif
  ...
  pac_debug("build_subtree: entering !cart_mode") ;
    ...
    if (axis_parallel) {
#ifdef _DEBUG_
  pac_debug("build_subtree: it is axis_parallel") ;
#endif
      cur_impurity = axis_parallel_split(cur_points, cur_no_of_points) ;
#ifdef _DEBUG_
  pac_debug("build_subtree: axis_parallel_split() done") ;
#endif
    }
...
#ifdef _DEBUG_
  pac_debug("build_subtree: it is oblique") ;
#endif
...
#ifdef _DEBUG_
  pac_debug("build_subtree: oblique_split() done") ;
#endif
...
#ifdef _DEBUG_
  pac_debug("build_subtree: Can not find any split given current parameter settings") ;
#endif
...
#ifdef _VERBOSE_
  if (strlen(node_str)) fprintf(logfile,"** \"%s\": ",node_str);
  else fprintf(logfile,"** Root: ");
  fprintf(logfile,"Left:[");
  for (i=1; i < no_of_categories; i++) fprintf(logfile,"%d,",left_count[i]);
  fprintf(logfile,"%d] Right:[",left_count[no_of_categories]);
  for (i=1; i < no_of_categories; i++) fprintf(logfile,"%d,",right_count[i]);
  ...
  if (left_count[cur_node->left_cat] != lpt) {
	...
	== NULL) pac_fatal("BUILD_DT : Memory allocation failure.");
    lpoints-- ;
    lindex = 0 ;
  }
     ...
     char *cur_label ;
{
  ...
  */
  ...
  while (TRUE) {
...
#ifdef _VERYVERBOSE_
	fprintf(logfile,"\tCART hill climbing for coeff. %d. impurity %.3f -> %.3f\n",
    ...
    if (cycle_count != 1 && fabs(prev_impurity - cur_error)<TOLERANCE)
      break;
  }
...
struct tree_node *create_tree_node(void)
{
  ...
  */
  ...
  if (cur_node == NULL) pac_fatal("Create_Tree_Node : Memory allocation failure.") ;
	...
	if (new_error <= cur_error &&
...
#ifdef _VERYVERBOSE_
	  fprintf(logfile,"\thill climbing for coeff. %d. impurity %.3f -> %.3f\n",
		  cur_coeff, cur_error, new_error) ; 
#endif      
	  ...
	  while (cur_error != 0 &&
	    ...
	    new_error = perturb_randomly
	      (cur_points, cur_no_of_points, cur_error, 0);
	    if (alter_coefficients(cur_points, cur_no_of_points)) {

#ifdef _VERYVERBOSE_
	      fprintf(logfile,"\trandom jump. impurity %.3f -> %.3f\n",
		      cur_error, new_error) ;
#endif      
...
/************************************************************************/
int normalize_data(points,no_of_points)
      struct point **points ;
      int no_of_points ;
{
 int i, j ;
...
/************************************************************************/
int unnormalize_data(points, no_of_points)
     struct point **points ;
     int no_of_points ;
{
  int i, j ;

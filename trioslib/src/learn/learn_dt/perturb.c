...
float suggest_perturbation(cur_points,cur_no_of_points,cur_coeff,cur_error)
     ...
     int cur_no_of_points,cur_coeff ;
{
  int i, /*j, lpt, rpt,*/ no_of_eff_points=0 ;
  float d_dim_error ;
  /*
     ...
     char *cur_label ;
{
  ...
  rvector = fvector(1,no_of_coeffs) ;
  ...
  if (!no_of_eff_points) {
    free_fvector(rvector,1,no_of_coeffs) ;
    return(FALSE) ;
  }
     ...
     int no_of_eff_points ;
{
  ...
  candidates += 1 ;
  qsort((char *)candidates,(unsigned int) no_of_eff_points,sizeof(struct unidim),(qsort_compare_type) compare) ;
  candidates -= 1 ;

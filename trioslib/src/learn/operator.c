...
image_operator_t *image_operator_build_bb(imgset_t * set, window_t * win)
{
	...
	if (iop->bb == NULL) {
		return (image_operator_t *) trios_error(MSG,
							"Error in lisi_partitioned");
	}

	return iop;
}

static image_operator_t *build_with_tree(imgset_t * set, window_t * win,
					 int type, int decision)
{
	image_operator_t *iop;
	trios_malloc(iop, sizeof(image_operator_t), image_operator_t *,
		     "Failed to alloc image operator");
...
image_operator_t *train_wk(image_operator_t * iop, imgset_t * set,
			   window_t * win)
{
	iop->bb = NULL;
...
image_operator_t *image_operator_build_wkc(imgset_t * set, window_t * win,
					   int ki, int vplace)
{
...
image_operator_t *image_operator_build_wkf(imgset_t * set, window_t * win,
					   int ki, int ko, int vplace)
{
	apert_t *apt = apert_create(1);
	apert_set_band_ki(apt, 1, ki);
...
image_operator_t *image_operator_build_wk(imgset_t * set, window_t * win,
					  apert_t * apt)
{
	image_operator_t *iop;
	trios_malloc(iop, sizeof(image_operator_t), image_operator_t *,

  ...
  *y=ya[ns--];
  for (m=1;m<n;m++) {
    for (i=1;i<=n-m;i++) {
      ...
      hp=xa[i+m]-x;
      w=c[i+1]-d[i];
      if ( (den=ho-hp) == 0.0) pac_fatal("Error in routine polint");
      den=w/den;
      d[i]=hp*den;
      c[i]=ho*den;
    }
...
/* allocate a float vector with subscript range v[nl..nh] */
{
  ...
  if (!v) pac_fatal("allocation failure in vector()");
  return v-nl+NR_END;
}
...
float *fvector(int nl, int nh) 
{
  ...
  if (v==NULL) pac_fatal("Vector : Memory allocation failure.") ;
  return (v-nl) ;
}
...
int *ivector(int nl, int nh)
{
  ...
  if (v==NULL) pac_fatal("Ivector : Memory allocation failure.") ;
  return(v-nl) ;
}
...
double *dvector(int nl, int nh)
{
  ...
  if (v==NULL) pac_fatal("Dvector : Memory allocation failure.") ;
  return (v-nl) ;
}
...
void splint(float xa[], float ya[], float y2a[], int n, double x, float *y)
{
  ...
  h = xa[khi]-xa[klo] ;
  if (h == 0.0) pac_fatal("Bad xa input to routine splint") ;
  a = (xa[khi]-x)/h ;
  b = (x-xa[klo])/h ;
...
/* allocate a float matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  if (!m) pac_fatal("allocation failure 1 in matrix()");
  m += NR_END ;
  m -= nrl ;

  /* allocate rows and set pointers to them */
  m[nrl]=(float *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float))) ;
  if (!m[nrl]) pac_fatal("allocation failure 2 in matrix()") ;
  m[nrl] += NR_END ;
  m[nrl] -= ncl ;
...
)
{
  ...
  pac_fatal("Too many steps in routine qsimp");
  return 0.0;
}
...
)
{
  ...
  pac_fatal("Too many steps in routine qromb");
  return 0.0;
}

...
multi_architecture_t *multi_level_arch_create(int nlevels, int *operators_in_level) {
    ...
    if (nlevels < 1) {
        return (multi_architecture_t *) trios_error(MSG, "Cannot create operator with less than one level.");
    }
    for (i = 0; i < nlevels; i++) {
        if (operators_in_level[i] <= 0) {
            return (multi_architecture_t *) trios_error(MSG, "Level %d has less than 1 operator.", operators_in_level[i]);
        }
    }
    if (operators_in_level[nlevels-1] != 1) {
        return (multi_architecture_t *) trios_error(MSG, "Last level MUST have only one operator.");
    }
    ...
    if (m->levels == NULL) {
        return (multi_architecture_t *) trios_error(MSG, "Bad Alloc");
    }
...
multi_level_operator_t *multi_level_operator_create(multi_architecture_t *m, int type) {
    ...
    for (i = 0; i < m->nlevels; i++) {
        ...
        for (j = 0; j < op->levels[i].noperators; j++) {
            op->levels[i].trained_operator[j] = NULL;
...
window_t *multi_level_operator_get_window(multi_level_operator_t *mop, int level, int op, int input) {
    if (mop == NULL) {
        return (window_t *) trios_error(MSG, "Operator pointer is null");
    }
    if (level >= mop->nlevels) {
        return (window_t *) trios_error(MSG, "Operator has less than %d levels", level);
    }
    if (op >= mop->levels[level].noperators) {
        return (window_t *) trios_error(MSG, "Level %d contains less than %d operators.", op);
    }
    if (input >= mop->levels[level].ninputs) {
        return (window_t *) trios_error(MSG, "Operator %d in level %d contains less than %d inputs.", op, level, input);
    }

...
trios_debug("Vai chamar o xpl_GG_create");
    ...
    *pp = p = xpl_GG_create(xpl->wsize) ;
    if(p == NULL) {
      (void)trios_error(MSG, "xpl_GG_insert: xpl_GG_create() failed.") ;
      return(-1) ;
    }
    xpl->n_nodes++ ;
...
#ifdef _DEBUG_
trios_debug("O w-pattern e' igual, vai ajustar as frequencias");
#endif

      xpl->sum += freq_sum(freqlist) ;

      if (!set_freq(freqlist, &p->freqlist)) {
    (void)trios_error(MSG, "xpl_GG_insert: set_freq() failed.") ;
    return(-1) ;
      }
    ...
    trios_debug("%d %d ", pq->label, pq->freq) ;
      ...
      if(xpl_GG_insert(xpl, &p->right, wpat, freqlist)) {
        ...
        if(p->bal == 1) deltaH = 1 ;
          ...
          if(p->right->bal == -1) GG_rightrotate(&p->right) ;
          GG_leftrotate(pp) ;
        }
      }
    }
      ...
      if(xpl_GG_insert(xpl, &p->left, wpat, freqlist)) {
        ...
        if(p->bal == -1) deltaH = 1 ;
          ...
          if(p->left->bal == 1) GG_leftrotate(&p->left) ;
          GG_rightrotate(pp) ;
        }
      }
    }
...
/*  date Mon May  5 1997                                                    */
  ...
  if(p != NULL) {
    ...
    if(!xpl_GG_merge(xpl, p->left)) {
      return trios_error(MSG, "Recursive call to xpl_GG_merge() failed.") ;
    }
    ...
    if(xpl_GG_add(xpl, (xpl_GG **)(&xpl->root), p)==-1) {
      return trios_error(MSG, "xpl_GG_merge: xpl_GG_add() failed.");
    }
...
/*  date Wed May  7 1997                                                    */
  ...
  if(p==NULL) {
    ...
    *p_cur = p_new ;
    ...
    (void)trios_error(MSG, "xpl_GG_add: set_freq() failed.") ;
    return(-1) ;
      }
      p_new->freqlist = NULL ;
      xpl_GG_free(p_new) ;
...
/*  date: Fri Nov 29 1996                                                   */
  ...
  if(p!=NULL) {
    if(!tree2ll_GG_asc(p->right, pp)) {
      return trios_error(MSG, "Recursive call to tree2ll_GG_asc() failed.") ;
    }
    p->right = *pp ;
    *pp = p ;
...
/*  date: Fri Nov 29 1996                                                   */
  ...
  if(p!=NULL) {
    if(!tree2ll_GG_desc(p->left, pp)) {
      return trios_error(MSG, "Recursive call to tree2ll_GG_desc() failed.") ;
    }
    p->left = *pp ;
    *pp = p ;
...
/*  date: Fri Nov 29 1996                                                   */
  ...
  if(p != NULL) {
    if(!xpl_GG_sort(p->left, pp)) {
      return trios_error(MSG,"Recursive call to xpl_GG_sort() failed.");
    }
...
/*  date: Fri Nov 29 1996                                                   */
  ...
  if(q == NULL) {
    ...
    if(q->bal == 1) deltaH = 1 ;
      ...
      if(q->right->bal == -1) GG_rightrotate(&q->right) ;
      GG_leftrotate(pp) ;
    }
      }
    }
    ...
    if(q->bal == -1) deltaH = 1 ;
      ...
      if(q->left->bal == 1) GG_leftrotate(&q->left) ;
      GG_rightrotate(pp) ;
    }
      }
    }

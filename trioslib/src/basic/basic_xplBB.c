    ...
    if(st == 0) {
      ...
      if(xpl_BB_insert(xpl, &p->right, wpat, fq0, fq1)) {
        ...
        if(p->bal == 1) deltaH = 1 ;
          ...
          if(p->right->bal == -1) BB_rightrotate(&p->right) ;
          BB_leftrotate(pp) ;
        }
      }
    }
      ...
      if(xpl_BB_insert(xpl, &p->left, wpat, fq0, fq1)) {
        ...
        if(p->bal == -1) deltaH = 1 ;
          ...
          if(p->left->bal == 1) BB_leftrotate(&p->left) ;
          BB_rightrotate(pp) ;
        }
      }
    }
...
/*  date: Fri Nov 29 1996                                                   */
  ...
  if(q == NULL) {
	...
	if(q->bal == 1) deltaH = 1 ;
	  ...
	  if(q->right->bal == -1) BB_rightrotate(&q->right) ;
	  BB_leftrotate(pp) ;
	}
      }
    }
	...
	if(q->bal == -1) deltaH = 1 ;
	  ...
	  if(q->left->bal == 1) BB_leftrotate(&q->left) ;
	  BB_rightrotate(pp) ;
	}
      }
    }

...
long				/*+ Purpose: generates a seed to be used for random
				   number generation                             + */ pac_gen_seed(void
    )
...
float				/*+ Purpose: Generate a uniform random deviate between
				   0.0 and 1.0 (exclusive of the endpoint values)        + */ ran1(
...
float /*+ Purpose: generate a random number                     + */ ran2x(
...
/*  date: Thu Feb 26 1998                                                   */
	...
	*idum = IA1 * (*idum - k * IQ1) - k * IR1;	/*Compute idum(IA1*idum)%IM1 without */
	if (*idum < 0)
		*idum += IM1;	/* overflows by Scharge's method */
	k = idum2x / IQ2;
	...
	if (iyx < 1)
		iyx += IMM1;
	if ((temp = AM1 * iyx) > RNMX)
		return RNMX;	/* Because users dont expect
				   endpoint values. */
	else
		return temp;
}
...
float /*+ Purpose: generate a random number                     + */ ran2y(
										  long *idum	/*+ In/Out: current seed                            + */
    )
/*+ Return: a random number                                                +*/
{
...
/* allocate a float vector with subscript range v[nl..nh] */
{
	...
	if (!v)
		trios_fatal("allocation failure in vector()");
	return v - nl + NR_END;
}
	...
	*y = ya[ns--];
	for (m = 1; m < n; m++) {
		for (i = 1; i <= n - m; i++) {
			...
			w = c[i + 1] - d[i];
			if ((den = ho - hp) == 0.0)
				trios_fatal("Error in routine polint");
			den = w / den;
			d[i] = hp * den;
			c[i] = ho * den;
		}
...
float *fvector(int nl, int nh)
{
	...
	if (v == NULL)
		trios_fatal("Vector : Memory allocation failure.");
	return (v - nl);
}
...
int *ivector(int nl, int nh)
{
	...
	if (v == NULL)
		trios_fatal("Ivector : Memory allocation failure.");
	return (v - nl);
}
...
double *dvector(int nl, int nh)
{
	...
	if (v == NULL)
		trios_fatal("Dvector : Memory allocation failure.");
	return (v - nl);
}
...
float trapzd(float (*func) (double, double, double),
	     double a, double b, int n, double pa, double pb)
{
	float x, tnm, sum, del;
	static float s;
	int it, j;
	...
	if (n == 1) {
		return (s =
			0.5 * (b - a) * (FUNC(a, pa, pb) + FUNC(b, pa, pb)));
	} else {
...
void spline(float x[], float y[], int n, double yp1, double ypn, float y2[])
{
	...
	for (i = 2; i <= n - 1; i++) {
		...
		y2[i] = (sig - 1.0) / p;
		u[i] =
		    (y[i + 1] - y[i]) / (x[i + 1] - x[i]) - (y[i] -
							     y[i - 1]) / (x[i] -
									  x[i -
									    1]);
		u[i] =
		    (6.0 * u[i] / (x[i + 1] - x[i - 1]) - sig * u[i - 1]) / p;
	}
	if (ypn > 0.99e30)
		qn = un = 0.0;
	else {
		qn = 0.5;
		un = (3.0 / (x[n] - x[n - 1])) * (ypn -
						  (y[n] - y[n - 1]) / (x[n] -
								       x[n -
									 1]));
	}
...
void splint(float xa[], float ya[], float y2a[], int n, double x, float *y)
{
	...
	h = xa[khi] - xa[klo];
	if (h == 0.0)
		trios_fatal("Bad xa input to routine splint");
	a = (xa[khi] - x) / h;
	b = (x - xa[klo]) / h;
	...
	*nfunk = 0;
	GET_PSUM for (;;) {
		...
		*nfunk += 2;
		...
		if (ytry <= y[ilo])
			ytry =
			    amotry(p, y, psum, ndim, funk, ihi, 2.0, px, npx);
		else if (ytry >= y[inhi]) {
			ysave = y[ihi];
			ytry =
			    amotry(p, y, psum, ndim, funk, ihi, 0.5, px, npx);
			if (ytry >= ysave) {
				for (i = 1; i <= mpts; i++) {
					if (i != ilo) {
						for (j = 1; j <= ndim; j++)
...
/* allocate a float matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	...
	if (!m)
		trios_fatal("allocation failure 1 in matrix()");
	m += NR_END;
	m -= nrl;
	...
	if (!m[nrl])
		trios_fatal("allocation failure 2 in matrix()");
	m[nrl] += NR_END;
	m[nrl] -= ncl;
	for (i = nrl + 1; i <= nrh; i++)
		m[i] = m[i - 1] + ncol;
...
float qsimp(float (*func) (double, double, double),
	    double a, double b, double pa, double pb)
{
	int j;
	float s, st, ost, os;

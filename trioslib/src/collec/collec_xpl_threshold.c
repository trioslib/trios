...
trios_debug("xpl structure have been allocated") ;
  ...
  for(j=0; j<npixels; j++) {    /* shifts the window through each pixel */

    if(p3[j] != 0) {          /* mask condition satisfied */

      for(k=0; k<wsize; k++) wpat[k] = 0 ;     /* blank w-pattern */

      /* gets the w-pattern centered at the point j */
      for(k=0; k<wsize; k++) {  /* for each point of the window...*/
    i = j+offset[k] ;
    if (i >= 0 && i < npixels)
        wpat[k] = (int)s1[i] ;
      }
      ...
      for(i=1; i<wsize+2; i++) {
        aux = ord[i] - ord[i-1] ;
        if(aux>0) {
          for(k=0; k<xpl->wzip; k++) bpat[k] = 0 ;
      for(k=0; k<wsize; k++) {
            if(wpat[k]>=ord[i]) {
          s = k/NB ;
          nshifts = k%NB ;
          bpat[s] = (bpat[s] | bitmsk[nshifts]) ;
        }
          }
          if((int)p2[j] >= ord[i]) {
            freq0 = 0 ;
...
#ifdef _DEBUG_
          if((j>2000) && (j<2100)){
            trios_debug("It will insert pattern %x freq0=%d freq1=%d", bpat[0], freq0, freq1) ;
          }
#endif

          st = xpl_BB_insert(xpl, (xpl_BB **)(&xpl->root), bpat, freq0, freq1) ;
          if(st == -1) {
            xpl_free(xpl) ;
            free(wpat) ;
...
/* date: Mon Apr 06 1999                                                    */
  ...
  if(imgset_get_grpsize(imgset)<2) {
    imgset_free(imgset) ;
    return trios_error(1, "Each image group must contain at leat two image files.") ;
  }
...
trios_debug("Starting image scanning") ;
  ...
  for(k=1; k<=imgset_get_ngroups(imgset); k++) {
    ...
    if(!get_setofimages(imgset, BB, win, k, &img1, &img2, &img3)) {
      ...
      free(offset) ;
      imgset_free(imgset) ;
      return trios_error(MSG, "lcolthresh: get_images() failed.") ;
    }
    nc = img_get_width(img1) ;
    npixels = img_get_width(img1) * img_get_height(img1) ;

    /* set vector of offsets (note that for each image group it may vary) */
    ...
    xpl_new = colthresh_BB(s1, c2, c3, offset, wsize, npixels, maxgrey) ;
    if(xpl_new==NULL) {
      return trios_error(MSG, "lcolthresh: colthresh_BB() failed.") ;
    }
...
trios_debug("End of image scanning") ;
  ...
  /* ACRESCIDO SOMENTE PARA GERAR ARQUIVO NO FORMATO SWT */
  swt_write("swt.dat", xpl, win) ;

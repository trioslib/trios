			   ...
			   int n_images)
{
	...
	if (level >= mop->nlevels) {
		return (xpl_t *) trios_error(MSG,
					     "Operator has less than %d levels",
					     level);
	}
	if (op >= mop->levels[level].noperators) {
		return (xpl_t *) trios_error(MSG,
					     "Level %d contains less than %d operators.",
					     op);
	}
	...
	for (i = 0; i < n_images; i++) {
		if (mop->type == BB) {
			xpl_new =
			    collec_level_operator_bb_main(mop, level, op,
							  input[i], mask[i],
							  ideal[i]);
		} else {
			xpl_new =
		...
		if (xpl == NULL) {
			...
			if (!xpl_merge(xpl_new, xpl)) {
				return (xpl_t *) trios_error(MSG,
							     "multi-level collec: merge error");
			}
			xpl_free(xpl);
			xpl = xpl_new;
			xpl_new = NULL;
				     ...
				     img_t * ideal)
{
	...
	for (k = 0; k < npixels; k++) {
		...
		/* mask condition */
		if (mask != NULL
		    && img_get_pixel(mask, k / mask->width, k % mask->width,
				     0) == 0) {
			continue;
		}
		for (i = 0; i < mop->levels[level].ninputs; i++) {
			offset_set(offset,
				   multi_level_operator_get_window(mop, level,
								   op, i),
				   img_get_width(inputs[i]), 1);
			for (j = 0;
			     j <
				...
				if (l >= 0 && l < npixels) {
					if (img_get_pixel
					    (inputs[i], l / inputs[i]->width,
					     l % inputs[i]->width, 0) != 0) {
						int index =
						    (j + curr_win_size) / NB;
						int bit =
						    (j + curr_win_size) % NB;
		...
		/* insere na arvore. */
		if (ideal->data[k] == 0) {
			xpl_BB_insert(xpl, (xpl_BB **) & xpl->root, joint_wpat,
				      1, 0);
		} else {
			xpl_BB_insert(xpl, (xpl_BB **) & xpl->root, joint_wpat,
				      0, 1);

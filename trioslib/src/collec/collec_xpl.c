...
int get_setofimages(imgset_t * imgset, int map_type, window_t * win, int k,
		    img_t ** img1, img_t ** img2, img_t ** img3)
{
	/* Read first image */
	...
	*img2 = img;
	...
	if (imgset_get_grpsize(imgset) >= 3) {
		...
		*img3 =
		    img_create(img_get_width(*img1), img_get_height(*img1), 1,
			       sz8BIT);
...
*/
int lcollec(char *fname_i1, char *fname_i2, char *fname_i3, int input_type,
	    int output_type, int cv_flag, char *fname_o1, char *fname_o2)
{
...
#ifdef _DEBUG_
	trios_debug("Starting lcollec %s e %s e cv=%d", fname_i1, fname_o1,
		    cv_flag);
#endif
	/* read image set file */
	...
	if (imgset_get_grpsize(imgset) < 2) {
		...
		return trios_error(1,
				   "Each image group must contain at least two image files.");
	}
	/* read WINSPEC file */
...
*/
int lcollec_main(imgset_t * imgset, window_t * win, xpl_t * xpl, int map_type,
		 int cv_flag, char *log_file)
{
	xpl_t *xpl_new = NULL;
	int k;
	int *offset;
	...
	if (log_file) {
		fd = fopen(log_file, "w");
		if (!fd) {
			return trios_error(1, "File (%s) open failed.",
					   log_file);
		}
	...
	if (offset == NULL) {
		return trios_error(MSG,
				   "lcollec_main: offset_create() failed.");
	}
	/* ---------------------------------------------------------------- */
	...
	for (k = 1; k <= imgset_get_ngroups(imgset); k++) {
		...
		if (map_type == BB) {
			...
			if (!get_setofimages
			    (imgset, BB, win, k, &img1, &img2, &img3)) {
				trios_error(MSG,
			...
			xpl_new =
			    collec_BB(s1, c2, c3, offset, wsize, npixels,
				      cv_flag);
			if (xpl_new == NULL) {
				trios_error(MSG,
					    "lcollec_main: collec_BB() failed.");
				goto END_lcollec_main;
			}
		} else if (map_type == BG) {

...
int select_number_of_first_level_operators(xpl_t *xpl2nd, point_weight *ranking, int n_windows, xpl_t **level2_new) {
    int i, j, *to_remove, *to_remove_iter;
    xpl_t *temp = NULL;
    window_t *w = NULL;
    ...
    *level2_new = temp;
    ...
    w = win_create(1, n_windows - i, 1);
    for (j = 0; j < n_windows - i; j++) {
        win_set_point(0, j, 1, 1, w);
    }
    xpl_write("xpl2ndnew", temp, w, NULL);
...
multi_level_operator_t *window_two_level_relief(window_t *domain, imgset_t *T1, imgset_t *T2, int n_1st_level, int sz_1st_level, int n_proto1, int n_proto2, int type) {
    image_operator_t **ops, **ordered_ops;
    ...
    fprintf(stderr, "Collec started...\n");
    xpl_domain = lcollec_memory(T1, domain, type);
    fprintf(stderr, "Collec finished...\n");
    
    for (i = 0; i < n_1st_level; i++) {
        fprintf(stderr, "Relief: %d, seed: %d\n", i, 100*i + i);
    ...
    for (i = 0; i < n_1st_level; i++) {
        fprintf(stderr, "Training operator %d\n", i);
        if (type == BB) {
    ...
    fprintf(stderr, "Collec for 2nd level started...\n");
    level2 = lcollec_two_level(T2, ops, n_1st_level);
    fprintf(stderr, "Collec for 2nd level finished...\n");
    
    temp = win_create(1, n_1st_level, 1);
    for (i = 0; i < n_1st_level; i++) {
        win_set_point(0, i, 1, 1, temp);
    }
    
    xpl_write("xpl2ndlevel", level2, temp, NULL);
    
    fprintf(stderr, "Relief for 2nd level started...\n");
    temp2 = window_relief(level2, temp, n_proto2, 1, &operator_ranking, 0);
    fprintf(stderr, "Selecting number of first level operators...\n");
    ...
    trios_malloc(ordered_ops, num1stlevel * sizeof(image_operator_t*), multi_level_operator_t*, "Failed to alloc array.");
    for (i = 0; i < num1stlevel; i++) {
        ordered_ops[i] = ops[operator_ranking[i].j];
    }
    result = multi_level_combine_xpl(ordered_ops, num1stlevel, level2_new);
    
    for (i = num1stlevel; i < n_1st_level; i++) {
        image_operator_free(ops[operator_ranking[i].j]);
    }
    free(ops);
    free(ordered_ops);
    free(wins);

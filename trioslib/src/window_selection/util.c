...
xpl_t *xpl_remove_variables(xpl_t * old_xpl, int *vars, int n_remove) {
    ...
    trios_malloc(idx, sizeof(int) * n_remove, xpl_t *, "Could not alloc aux array.");
    n = old_xpl->wsize;
    for (i = 0, k = 0; i < n; i++) {
        rem = 0;
    ...
    new_xpl = xpl_create(n - n_remove, new_xpl->type);
    trios_malloc(wpat, sizeof(int) * new_xpl->wzip, xpl_t *, "Could not alloc aux array.");
